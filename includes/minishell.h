...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/10 15:02:32 by krchuaip          #+#    #+#             */
/*   Updated: 2023/12/12 18:45:28 by krchuaip         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct							s_exec_list
{
	...
	t_exec_list					*prev;
	t_exec_list					*next;
	t_list_bi					*pipeline_commands;
	t_list_bi					*token_list_head;
	t_list						*envlist_head;
 ...
 * 	t_node_kind		type;			// command or subshell
	...
	t_list_bi		*redirect_list;	// set fd before execute commands
	int				redirect_fd[3];	// in, out, here_doc;
	pid_t			pid;
	t_list_bi		*pipeline_token_list; //tmp_save
	t_list_bi		*subshell_token_list; //execute -> parsing
 */
struct							s_command_info
{
	...
	int							redirect_fd[3];
	pid_t						pid;
	t_list_bi					*pipeline_token_list;
	t_list_bi					*subshell_token_list;
};
...
void							debug_print_exec_list(t_exec_list *head,
									char *str);
void							debug_print_redirect_info(t_list_bi *head,
									char *str);
