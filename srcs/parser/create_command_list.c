...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/10 14:58:24 by krchuaip          #+#    #+#             */
...
static int	connect_cmd_info_to_execlist(
		t_list_bi **connect_to, t_command_info *command_info)
{
	t_list_bi	*new_pipeline;

	new_pipeline = ft_lstnew_bi(command_info);
	if (!new_pipeline)
		return (FAILURE);
	ft_lstadd_back_bi(connect_to, new_pipeline);
	return (SUCCESS);
}
...
t_exec_list **pipe_node, t_command_info **cmd_info)
{
	...
	while ((*pipe_node)->token_list_head)
	{
		...
		&(*pipe_node)->pipeline_commands, *cmd_info) == FAILURE)
			return (FAILURE);
		ft_lstdelone_bi(&popped_token, free_token_elem);
