...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   create_expanded_commands.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: krchuaip <krittin@42bangkok.com>           +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/10 14:54:09 by krchuaip          #+#    #+#             */
...
static size_t	get_connected_token_size(t_list_bi *list)
{
	size_t			size;
...
static int	create_commands(t_list_bi **token_list, char ***commands)
{
	t_list_bi		*popped_list;
	t_token_elem	*token_elem;
	size_t			idx;
	...
	while (*token_list)
	{
		popped_list = ft_lstpop_bi(token_list);
		...
		if (!(*commands)[idx])
			return (FAILURE);
		idx++;
		ft_lstdelone_bi(&popped_list, free_token_elem);
	}
	return (SUCCESS);
}

/* token:[echo],[hello],[world] -> commands:{"echo", "hello", "world", NULL} */
static char	**create_commands_from_token_list(t_list_bi **token_list)
{
	char			**commands;
	size_t			size;
