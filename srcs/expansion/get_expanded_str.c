...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/10 14:54:40 by krchuaip          #+#    #+#             */
...
char	*concat_dst_to_src(char **dst, char *src)
{
	...
	if (!dst)
		return (NULL);
	dstlen = ft_strlen_nn(*dst);
	srclen = ft_strlen_nn(src);
	concat_str = (char *)ft_calloc(sizeof(char), dstlen + srclen + 1);
	if (!concat_str)
		return (perror_ret_nullptr("malloc"));
	ft_strlcat_nn(concat_str, *dst, dstlen + 1);
	ft_strlcat_nn(concat_str, src, dstlen + srclen + 1);
	free(*dst);
	*dst = NULL;
	return (concat_str);
}
