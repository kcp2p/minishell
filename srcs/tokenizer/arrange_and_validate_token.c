...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/29 18:55:59 by krchuaip          #+#    #+#             */
...
/*
 * Inspired by my friend at 42 Tokyo,
 *
 * ( () ( () () ) )
 * 0 11 1 22 22 1 0
*/

static void	set_parenthesis_no(t_list_bi **tokenlist_head)
{
	t_list_bi		*node;
	t_token_elem	*token;
	int				parentesis_no;
...
static void	set_elem_type_if_operator(t_list_bi **tokenlist_head)
{
...
static int	set_token_parameter(t_list_bi **node)
{
	t_token_elem	*token;
...
static int	set_elem_type_if_word(t_list_bi **tokenlist_head)
{
	t_list_bi		*node;
...
int	arrange_and_validate_token_list(t_list_bi **tokenlist_head)
{
	...
	if (ft_lstsize_bi(*tokenlist_head) == 0)
		return (PROCESS_ERROR);
	return (EXIT_SUCCESS);
}
