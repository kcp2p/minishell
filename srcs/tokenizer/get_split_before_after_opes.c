...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/29 18:56:12 by krchuaip          #+#    #+#             */
...
char *word, t_list_bi **list_head, t_split_info *s)
{
	...
	new_list = ft_lstnew_bi(token_elem);
	if (!new_list)
	{
		free_token_elem(token_elem);
		return (perror_ret_int("malloc", FAILURE));
	}
	ft_lstadd_back_bi(list_head, new_list);
	return (SUCCESS);
}

static int	get_splitted_word_to_list(\
t_list_bi **splitted_list_head, t_split_info *s)
{
	char	*splittd_word;
...
const char *src, const char *opes, char *quote)
{
	...
	while (src[s_info->head_idx])
	{
		...
		{
			ft_lstclear_bi(&splitted_list_head, free_token_elem);
			free(s_info);
